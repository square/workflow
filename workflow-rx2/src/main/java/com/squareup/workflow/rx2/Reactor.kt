...
/**
 * An Rx2 adapter for [com.squareup.workflow.Reactor], allowing implementors
 * to ignore that interface's reliance on [kotlin.coroutines].
 *
 ...
 * to [onReact], along with an [EventChannel] that can be used to accept events,
 ...
 *   Emit [EnterState.state][[com.squareup.workflow.EnterState.state]
 *   from [Workflow.state], and pass it as the next state value of a
 *   new call to [onReact].
 *
 *   - [FinishWith][com.squareup.workflow.FinishWith]: Fire [Workflow.result] with
 *   [FinishWith.result][com.squareup.workflow.FinishWith.result], and terminate
 *   the workflow.
 *
 * To handle events received from [Workflow.sendEvent], call
 * [`events.select`][EventChannel.select] from within your
 * [onReact] method:
 *
 ...
 *    ): Single<Reaction<MyState, MyResult>> {
 *      return when(state) {
 ...
 *    }
 *
 * [onReact] is not limited to using the given [EventChannel] to calculate its
 * next state. For example, a service call might be handled this way, mapping
 * a [Single] generated by Retrofit to the appropriate [Reaction].
 *
 *    override fun onReact(
 *      state: MyState,
 *      events: EventChannel<MyEvent>,
 ...
 *    }
 *
 * If you need to mix such command-like [Single]s with workflow events, [EventChannel] offers
 * `onSuccess` in addition to `onEvent`:
 *
 *    override fun onReact(
 *      state: MyState,
 *      events: EventChannel<MyEvent>,
 ...
 *
 *          onSuccess(statusService.update) { response ->
 *            if (response.success) EnterState(ShowingStatus(response))
 *            else EnterState(ShowingFailure(response)
 *          }
 *
 *          onEvent<Cancel> { EnterState(ShowingFailure(Canceled()) }
 *        }

 *      // ...
 *    }
 *
 * To monitor external hot observables, which might fire at any time, subscribe
 * to them in the [launch] method, and map their values to events passed to [Workflow.sendEvent].
 * Use [Workflow.toCompletable][com.squareup.workflow.rx2.toCompletable] to tear down those
 ...
 * represents it implement [com.squareup.workflow.Delegating].
 * Use [WorkflowPool.nextDelegateReaction] when entering
 ...
 *
 *    is Delegating -> workflows.nextDelegateReaction(state).map {
 *      when (it) {
