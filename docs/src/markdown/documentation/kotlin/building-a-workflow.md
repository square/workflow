...
```kotlin
class DemoWorkflow : Workflow<String, Unit, Nothing, String> {

    override fun initialState(input: String) = Unit
...
A class implementing `Workflow` represents a single node in the workflow tree. It may store resources like network services as properties (e.g. provided via dependency injection). Configuration values that may change over the lifetime of the workflow should be provided through the `InputT` type (a simple `String` in the example above).

Every workflow defines its own `StateT` type to contain any data that should persist through subsequent compose passes.
