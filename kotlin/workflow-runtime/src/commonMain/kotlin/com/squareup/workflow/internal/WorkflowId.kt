...
internal constructor(
   ...
   * ).
   */
  val typeDebugString: String = type.name()
}
...
internal fun WorkflowId<*, *, *>.toByteString(): ByteString = Buffer()
    .also { sink ->
      sink.writeUtf8WithLength(type.name())
...
internal fun restoreId(bytes: ByteString): WorkflowId<*, *, *> = bytes.parse { source ->
  ...
  val type = kClassForName<Workflow<Nothing, Any, Any>>(typeName)
  val name = source.readUtf8WithLength()
  return WorkflowId(type, name)
}

expect inline fun <reified T:Any> kClassForName(typeName: String):KClass<out T>

expect fun <T:Any> KClass<T>.name():String